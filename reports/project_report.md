...
The primary approach was to examine the raw distributions of core numerical continuous variables (e.g., `fare_amount`, `trip_distance`, `trip_duration_min`). Given financial data inherently suffers from severe positive right-skew (containing extreme high-value outliers and illogical negative records), standard descriptive statistics (`describe()`) and specific quantile approximations (`approxQuantile`) were calculated to establish mathematically justified clipping boundaries.

*Modelled Function (Distribution Profiling):*
```python
# Discussed Approach: Extracting the 1% and 99% quantiles to define statistical 
...
```python
# Discussed Approach: Assembling numerical features into a single Vector to 
...
### 3.3 Silver Tier: Vectorized Cleaning & Feature Construction
...
### 3.4 Gold Tier: Business-Level Aggregates
